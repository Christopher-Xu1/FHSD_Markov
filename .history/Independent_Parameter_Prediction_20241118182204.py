...
# Random values (if needed elsewhere)
...
# Mapping of transitions to their corresponding rates
# The key is (From_State, To_State)
transition_rates = {
    ("S", "S"): None,  # Self-transition
    ("S", "E"): f"VD={VD}",
    ("S", "I"): None,
    ("S", "R"): f"Δ={Δ}",
    ("S", "D"): None,
    ...
    ("E", "S"): f"d0={d0}",
    ("E", "E"): f"1 - (d0 + VT * TD + Δ)={1 - (d0 + VT * TD + Δ):.5f}",
    ("E", "I"): f"VT * TD + Δ={VT * TD + Δ:.5f}",
    ("E", "R"): None,
    ("E", "D"): None,

    ("I", "S"): None,
    ("I", "E"): None,
    ("I", "I"): f"1 - (d0 + Dr)={1 - (d0 + Dr):.5f}",
    ("I", "R"): f"d0={d0}",
    ("I", "D"): f"Dr={Dr:.5f}",
...
# Function to format each cell with probability and rate
def format_cell(prob, rate):
    prob_str = f"{prob:.5f}"
    if rate:
        return f"{prob_str} ({rate})"
    else:
        return f"{prob_str}"

# Prepare the table data
table = []
for from_state, probabilities in transition_probabilities_hourly.items():
    row = [from_state]
    for to_state, prob in zip(states, probabilities):
        rate = transition_rates.get((from_state, to_state), None)
        cell = format_cell(prob, rate)
        row.append(cell)
    table.append(row)

# Define headers
headers = ["From \\ To"] + states
